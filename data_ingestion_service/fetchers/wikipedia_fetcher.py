...
class WikipediaFetcher(BaseFetcher):
    ...
    async def fetch_country_info(self, country_name: str) -> List[Dict[str, Any]]:
        ...
        if response:
            ...
            # Save Markdown file if output_dir is set
            if self.output_dir:
                save_markdown_file(
                    self.output_dir,
                    self.source_name,
    ...
    async def fetch_attractions(self, location: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        Note: Wikipedia doesn't have dedicated "attractions" pages.
        All information is in the main article, which is fetched via fetch_country_info.
        This method returns an empty list to avoid 404 errors.
        """
        # Wikipedia doesn't have separate pages for attractions
        # All information is in the main article (fetched via fetch_country_info)
        return []
